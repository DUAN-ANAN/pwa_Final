 (function() {
     var IOS_REGEX, READ_ONLY_REGEX, SpatialiteFactory, SpatialitePlugin, SpatialitePluginTransaction, argsArray, nextTick, root, txLocks;
     ...
     SpatialitePlugin = function(openargs, openSuccess, openError) {
         var dbname;
         console.log("SpatialitePlugin openargs: " + (JSON.stringify(openargs)));
     ...
     SpatialitePlugin.prototype.databaseFeatures = {
         isSpatialitePluginDatabase: true
     };

     SpatialitePlugin.prototype.openDBs = {};

     SpatialitePlugin.prototype.addTransaction = function(t) {
     ...
     SpatialitePlugin.prototype.transaction = function(fn, error, success) {
         this.addTransaction(new SpatialitePluginTransaction(this, fn, error, success, true, false));
     };

     SpatialitePlugin.prototype.readTransaction = function(fn, error, success) {
         this.addTransaction(new SpatialitePluginTransaction(this, fn, error, success, true, true));
     };

     SpatialitePlugin.prototype.startNextTransaction = function() {
         var self;
         self = this;
         nextTick(function() {
             var txLock;
     ...
     SpatialitePlugin.prototype.close = function(success, error) {
         if (this.dbname in this.openDBs) {
             ...
             cordova.exec(null, null, "SpatialitePlugin", "close", [{
                 path: this.dbname
             }]);
         }
     };

     SpatialitePlugin.prototype.executeSql = function(statement, params, success, error) {
         ...
         this.addTransaction(new SpatialitePluginTransaction(this, myfn, myerror, mysuccess, false, false));
     };
     ...
     SpatialitePluginTransaction = function(db, fn, error, success, txlock, readOnly) {
     ...
     SpatialitePluginTransaction.prototype.start = function() {
         var err;
         try {
             this.fn(this);
             this.run();
     ...
     SpatialitePluginTransaction.prototype.handleStatementSuccess = function(handler, response) {
         var payload, rows;
         if (!handler) {
             return;
         }
     ...
     SpatialitePluginTransaction.prototype.handleStatementFailure = function(handler, response) {
         if (!handler) {
     ...
     SpatialitePluginTransaction.prototype.run = function() {
         ...
         cordova.exec(mycb, null, "SpatialitePlugin", mycommand, [{
             dbargs: {
     ...
     SpatialitePluginTransaction.prototype.abort = function(txFailure) {
     ...
     SpatialitePluginTransaction.prototype.finish = function() {
         var failed, succeeded, tx;
         if (this.finalized) {
             return;
         }
     ...
     SpatialiteFactory = {
         ...
         deleteDb: function(databaseName, success, error) {
             return cordova.exec(success, error, "SpatialitePlugin", "delete", [{
                 path: databaseName
             }]);
         }
     };

     root.SpatialitePlugin = {
         SpatialiteFeatures: {
             isSpatialitePlugin: true
         },
         openDatabase: SpatialiteFactory.opendb,
         deleteDatabase: SpatialiteFactory.deleteDb
     };
